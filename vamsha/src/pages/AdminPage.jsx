...
import { Users, Edit3, UserPlus, Save, CheckCircle, AlertCircle, LogOut, Camera, Moon, GitBranch, Sliders, X, Eye, EyeOff, ChevronDown, Download, Upload, ClipboardList } from 'lucide-react';
      ...
      <div className="admin-topbar">
        ...
        <nav className="admin-nav-horizontal">
          ...
          <div ref={dataDropdownRef} className="desktop-only-nav" style={{ position: 'relative' }}>
            <button
              className={`admin-nav-tab ${['single', 'bulk', 'photos', 'jyotisha', 'gotra', 'cloudinary'].includes(activeTab) ? 'active' : ''}`}
              onClick={() => setDataDropdownOpen(prev => !prev)}
              style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', borderBottom: ['single', 'bulk', 'photos', 'jyotisha', 'gotra', 'cloudinary'].includes(activeTab) ? '3px solid var(--color-gold)' : '3px solid transparent' }}
            >
            ...
            {dataDropdownOpen && (
              <div style={{
                ...
                <button
                  className={`admin-submenu-item ${activeTab === 'cloudinary' ? 'active' : ''}`}
                  onClick={() => { setActiveTab('cloudinary'); setDataDropdownOpen(false); }}
                >
                  <Upload size={16} /> Image Upload
                </button>
                <button
                  className={`admin-submenu-item ${activeTab === 'pending' ? 'active' : ''}`}
                  onClick={() => { setActiveTab('pending'); setDataDropdownOpen(false); }}
                >
          ...
          <button
            className={`admin-nav-tab mobile-only-nav ${activeTab === 'cloudinary' ? 'active' : ''}`}
            onClick={() => setActiveTab('cloudinary')}
          >
            <Upload size={18} /> Image Upload
          </button>
          <button
            className={`admin-nav-tab mobile-only-nav ${activeTab === 'pending' ? 'active' : ''}`}
      ...
      <div className="admin-content-wide">
        {activeTab === 'single' && (
          <SingleEditor
            ...
            onClearPendingImport={() => setPendingImportData(null)}
          />
        )}
        ...
        {activeTab === 'cloudinary' && (
          <CloudinaryUpload profiles={profiles} setProfiles={setProfiles} />
        )}

        {activeTab === 'pending' && (
          <PendingReview
            profiles={profiles}
            onReviewImport={(submission) => {
              setPendingImportData(submission);
